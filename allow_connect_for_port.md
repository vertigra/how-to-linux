...
Эта цепочка правил разрешить исходящее ssh соединение на порту 2500 (`$ ssh -p 2500 user@ip`). При этом, если на порту 2500 висит какая-то сетевая служба (`LISTEN`) то она по-прежнему останется недоступной(закрытой).  